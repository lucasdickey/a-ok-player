...
export type PodcastFeed =
  Database["public"]["Tables"]["podcast_subscriptions"]["Row"];
...
export async function addFeed(
  ...
  try {
    ...
    return {
      success: true,
      message: `Podcast added successfully with ${
        processingResult.episodeCount || 0
      } episodes`,
      feedId,
 ...
 */
export async function fetchAndStoreEpisodes(
  feedId: string,
  feedUrl: string
): Promise<FeedProcessingResult> {
  try {
    ...
    for (let i = 0; i < episodesToInsert.length; i += batchSize) {
      ...
      if (newEpisodes.length > 0) {
        const { error } = await supabase.from("episodes").insert(newEpisodes);
...
export async function refreshFeed(
  userId: string,
  feedId: string
): Promise<FeedProcessingResult & { podcast?: any }> {
  try {
    // First verify that the feed belongs to the user
    const { data: feed } = await supabase
      .from("podcast_subscriptions")
...
export async function refreshAllFeeds(userId: string): Promise<{
  ...
  try {
    ...
    const newEpisodes = results.reduce(
      (sum, r) => sum + (r.newEpisodeCount || 0),
      0
    );
...
export async function getFeedDetails(feedId: string) {
  ...
  try {
    ...
    console.log(
      `Successfully fetched feed details and ${episodes?.length || 0} episodes`
    );
    return {
      podcast,
      episodes: episodes || [],
